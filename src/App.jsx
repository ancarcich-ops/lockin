...
// ─── SAMPLE GAMES ─────────────────────────────────────────────────────────────
// Replace this array daily with real lines.
const SAMPLE_GAMES = [
...
export default function App() {
  ...
  useEffect(() => {
    async function load() {
      const { data: picksRows } = await supabase.from("picks").select("name, selections");
      if (picksRows) {
        const built = {};
        picksRows.forEach(row => { built[row.name] = row.selections; });
        setAllPicks(built);
      }
      const { data: resultsRows } = await supabase.from("group_results").select("key, result");
      if (resultsRows) {
        const built = {};
  ...
  async function markResult(key, result) {
    ...
    if (isClear) {
      ...
      await supabase.from("group_results").upsert({ key, result }, { onConflict: "key" });
        ...
        <div className="modal-overlay" onClick={() => setShowAdminModal(false)}>
          <div className="glass-card pop" onClick={e => e.stopPropagation()} style={{ borderRadius: 20, padding: "32px 28px", width: "100%", maxWidth: 340, border: "1px solid rgba(30,144,255,0.3)" }}>
            ...
            <input
              ...
              autoFocus
              style={{ width: "100%", borderRadius: 10, padding: "11px 14px", color: "#fff", fontSize: 14, fontFamily: "Outfit, sans-serif", marginBottom: 10, borderColor: adminError ? "rgba(248,113,113,0.6)" : undefined }}
            />
            {adminError && <div style={{ fontSize: 12, color: "#f87171", marginBottom: 10 }}>Wrong password</div>}
            <button onClick={submitAdminPassword} style={{ width: "100%", padding: "13px", background: "linear-gradient(135deg, #1E90FF, #0ea5e9)", border: "none", borderRadius: 12, color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: "Outfit, sans-serif" }}>
              Unlock
            </button>
          </div>
        </div>
      ...
      <div className="glass-nav" style={{ position: "sticky", top: 0, zIndex: 200 }}>
        <div style={{ maxWidth: 660, margin: "0 auto", padding: "0 22px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div className="logo-btn" onClick={handleLogoTap} style={{ display: "flex", alignItems: "center", gap: 11 }}>
            <div style={{ width: 36, height: 36, borderRadius: 11, background: isAdmin ? "linear-gradient(135deg, #f59e0b, #fbbf24)" : "linear-gradient(135deg, #1E90FF 0%, #0ea5e9 100%)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, boxShadow: isAdmin ? "0 4px 16px rgba(245,158,11,0.5)" : "0 4px 16px rgba(30,144,255,0.4)", transition: "all 0.3s" }}>
              {isAdmin ? "⚡" : "🎯"}
            </div>
            <div>
              <div style={{ fontSize: 16, fontWeight: 700, color: "#fff", letterSpacing: -0.3 }}>Lock In</div>
              <div style={{ fontSize: 9, color: isAdmin ? "rgba(251,191,36,0.7)" : "rgba(255,255,255,0.3)", letterSpacing: 0.8, marginTop: 1 }}>{isAdmin ? "ADMIN MODE" : TODAY}</div>
            </div>
          </div>
          ...
          <div className="fade-up">
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                {isAdmin && (
                  <div style={{ background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.4)", borderRadius: 10, padding: "6px 12px", fontSize: 11, color: "#fbbf24", fontWeight: 600 }}>⚡ Admin</div>
                )}
              </div>
            </div>
